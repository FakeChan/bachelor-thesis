function [Ek,e_main]=var_WindRadiasTheta_error(varname,command)
    clc;
    %========================================================================
    %计算坐标
    ...
    u_err=var_u_interp-nr_u_interp;
    v_err=var_v_interp-nr_v_interp;
    v_r=zeros(r,360);
    v_theta=v_r;
    for i=1:r
        for j=1:360
            v_r=u_err*cos(theta_new(i,j))+v_err*sin(theta_new(i,j));
            v_theta=-u_err*sin(theta_new(i,j))+v_err*cos(theta_new(i,j));
        end
    end
